...
func init() {
	android.RegisterModuleType("erika_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = erikaExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = erikaExpandVariables(ctx, dep_file)
	...
	cmd := erikaExpandVariables(ctx, String(g.properties.Cmd))
