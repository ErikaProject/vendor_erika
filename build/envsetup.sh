...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^erika_") ; then
        ERIKA_BUILD=$(echo -n $1 | sed -e 's/^erika_//g')
    else
        ERIKA_BUILD=
    fi
    export ERIKA_BUILD
...
function breakfast()
{
    target=$1
    local variant=$2
    source ${ANDROID_BUILD_TOP}/vendor/erika/vars/aosp_target_release
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the erika model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/erika-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.erika.device | grep -q "$ERIKA_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $ERIKA_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.erika.device | grep -q "$ERIKA_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.erika.device | grep -q "$ERIKA_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
